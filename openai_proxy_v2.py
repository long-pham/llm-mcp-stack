...
v2: Improved efficiency using deeper Anthropic SDK integration.
- Singleton client with connection pooling
- SDK stream iteration for richer event data + usage tracking in streaming
- Shared request builder (DRY)
- Pre-built static responses and JSON templates
...
Usage:
  ANTHROPIC_API_KEY=sk-... uv run python openai_proxy_v2.py
  OPENAI_PROXY_BACKEND=cli uv run python openai_proxy_v2.py
"""
...
def _build_anthropic_kwargs(body: dict[str, Any]) -> dict[str, Any]:
    ...
    Shared by both streaming and non-streaming paths to avoid duplication.
    """
...
# ---------------------------------------------------------------------------
# Anthropic API Backend (singleton client)
# ---------------------------------------------------------------------------
...
async def _anthropic_chat_stream(body: dict[str, Any]):
    ...
    Uses SDK's stream.text_stream for text chunks, then calls
    stream.get_final_message() to get accurate usage stats for
    the final SSE chunk.
    """
    kwargs = _build_anthropic_kwargs(body)
    model = kwargs["model"]
    ...
    async def event_generator():
        # Pre-build JSON envelope; splice content per token to avoid
        # full dict construction + json.dumps on every chunk.
        prefix = json.dumps({
            "id": comp_id,
        ...
        async with client.messages.stream(**kwargs) as stream:
            ...
            # Get the final accumulated message for usage + stop_reason
        ...
        yield {"data": json.dumps({
            ...
            "created": created,
            "model": model,
            "choices": [{"index": 0, "delta": {}, "finish_reason": _map_stop_reason(final_message.stop_reason)}],
            "usage": {
...
async def _cli_chat(body: dict[str, Any]) -> JSONResponse:
    ...
    model = _resolve_model(body.get("model"))
    prompt = _messages_to_prompt_string(body.get("messages", []))
